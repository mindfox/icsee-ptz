...
<style>
:root{{color-scheme:dark}}*{{box-sizing:border-box}}body{{font-family:system-ui,sans-serif;background:#111;color:#eee;margin:0;padding:18px}}main{{max-width:1180px;margin:auto}}.panel{{background:#1d1d1d;border:1px solid #333;border-radius:12px;padding:14px;margin-bottom:14px}}h1,h2{{margin:0 0 10px}}.small{{font-size:13px;color:#aaa;margin:0 0 12px}}.camera-row{{display:grid;grid-template-columns:minmax(0,1fr) 280px;gap:14px;align-items:start}}.feed-wrap{{position:relative;background:#000;border-radius:8px;overflow:hidden;min-height:260px}}.feed-wrap img{{display:none;width:100%;max-height:68vh;object-fit:contain;background:#000}}.feed-badge{{position:absolute;left:8px;bottom:8px;background:#000b;padding:4px 7px;border-radius:5px;font:12px ui-monospace,monospace}}.controls{{display:flex;flex-direction:column;gap:10px}}.grid{{display:grid;grid-template-columns:repeat(3,58px);gap:7px;justify-content:center}}.zoom,.preset-actions{{display:grid;grid-template-columns:1fr 1fr;gap:7px}}button{{font-size:20px;min-height:48px;border:1px solid #444;border-radius:8px;background:#333;color:#fff;cursor:pointer;padding:5px 9px}}button:hover{{background:#444}}button:active{{background:#666}}.zoom button,.restart,.preset-actions button,.feed-toggle{{font-size:13px;min-height:40px}}.step-box,.preset-box{{display:flex;flex-direction:column;gap:7px;background:#272727;border:1px solid #444;border-radius:8px;padding:8px 10px;font-size:13px}}.step-row{{display:flex;align-items:center;justify-content:space-between;gap:10px}}.step-box input{{width:72px;font-size:16px;padding:5px 4px;text-align:center}}.preset-box select{{width:100%;font-size:13px;padding:7px}}.restart{{background:#633}}.restart:hover{{background:#844}}.console{{height:280px;overflow:auto;background:#080808;border:1px solid #333;border-radius:8px;padding:10px;font:12px/1.45 ui-monospace,SFMono-Regular,Consolas,monospace;white-space:pre-wrap;word-break:break-word;color:#c9f7d2}}.console .error{{color:#ff9d9d}}.console .warning{{color:#ffd27d}}.status{{font:12px ui-monospace,monospace;color:#b8f7c5;min-height:34px;white-space:pre-wrap;word-break:break-word}}.footer-row{{display:flex;justify-content:space-between;gap:10px;align-items:center;margin-bottom:8px}}.footer-row h2{{margin:0}}.footer-row button{{font-size:12px;min-height:32px}}@media(max-width:760px){{.camera-row{{grid-template-columns:1fr}}.controls{{max-width:300px;margin:auto;width:100%}}.feed-wrap{{min-height:190px}}}}
</style></head><body><main>
<div class="panel"><h1>iCSee PTZ Lab</h1><p class="small">Feed is disabled by default. PTZ and preset recall use ONVIF.</p>
...
<div class="preset-box"><strong>Saved positions</strong><span class="small">Preset creation is disabled because this camera overwrites preset 1 when ONVIF does not receive a valid camera-specific token. Refresh and recall remain available.</span><select id="presetSelect"><option value="">Press Refresh list to load presets</option></select><div class="preset-actions"><button id="reloadPresets">Refresh list</button><button id="gotoPreset">Go to selected</button></div></div>
