...
def onvif_set_preset(name: str, preset_token: str) -> dict:
    ...
    body = f'''<?xml version="1.0" encoding="UTF-8"?>
<s:Envelope xmlns:s="{SOAP_NS}" xmlns:tptz="{TPTZ_NS}"><s:Body><tptz:SetPreset><tptz:ProfileToken>{profile}</tptz:ProfileToken>{name_xml}<tptz:PresetToken>{safe_token}</tptz:PresetToken></tptz:SetPreset></s:Body></s:Envelope>'''
    ...
    return {"status": response.status_code, "requested_token": preset_token, "preset_token": returned_token, "name": name}
    ...
    try:
        with onvif_lock:
            ...
            if not any(str(item.get("token")) == preset_token for item in existing):
                add_log("WARNING", f"Rejected SetPreset because token {preset_token!r} was not returned by GetPresets")
