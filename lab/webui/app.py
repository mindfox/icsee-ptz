...
def onvif_continuous_move(command: str, pulse_seconds: float) -> dict:
    ...
    velocity = (
        f'<tt:Zoom x="{zoom:g}" space="http://www.onvif.org/ver10/tptz/ZoomSpaces/VelocityGenericSpace"/>'
        if command.startswith("zoom_")
        else f'<tt:PanTilt x="{x:g}" y="{y:g}" space="http://www.onvif.org/ver10/tptz/PanTiltSpaces/VelocityGenericSpace"/>'
    )
    move_body = envelope(
...
<label class="step-box" for="ptzStep"><span class="step-row"><span>Movement step</span><input id="ptzStep" type="number" min="1" max="10" step="1" value="{DEFAULT_PTZ_STEP}"></span><span class="small">Step 1 = {PTZ_STEP_SECONDS:g}s pulse</span></label>
