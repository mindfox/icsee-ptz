...
<style>
body{font-family:system-ui,sans-serif;background:#111;color:#eee;margin:0;padding:20px}main{max-width:900px;margin:auto}.panel{background:#1d1d1d;border:1px solid #333;border-radius:12px;padding:16px;margin-bottom:16px}img{width:100%;max-height:62vh;object-fit:contain;background:#000;border-radius:8px}.grid{display:grid;grid-template-columns:repeat(3,80px);gap:10px;justify-content:center}.zoom{display:flex;gap:10px;justify-content:center;margin-top:14px}button{font-size:24px;min-height:64px;border:0;border-radius:10px;background:#333;color:#fff;cursor:pointer}button:hover{background:#444}button:active{background:#666}.status{font-family:ui-monospace,monospace;white-space:pre-wrap;margin-top:12px;color:#b8f7c5}.small{font-size:14px;color:#aaa}
...
</div><div class="zoom"><button data-cmd="zoom_in">Zoom +</button><button data-cmd="zoom_out">Zoom −</button></div><div id="status" class="status">Ready</div></div>
</main><script>
...
document.querySelectorAll('[data-cmd]').forEach(b=>b.onclick=()=>move(b.dataset.cmd));document.getElementById('refresh').onclick=refresh;image.onerror=()=>status.textContent='Snapshot failed; check container logs';refresh();setInterval(refresh,1500);
</script></body></html>"""
