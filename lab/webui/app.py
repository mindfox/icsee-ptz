...
def soap_post(url: str, action: str, body: str) -> requests.Response:
    response = requests.post(
        ...
        headers={"Content-Type": f'application/soap+xml; charset=utf-8; action="{action}"', "Connection": "close"},
        auth=AUTH, timeout=ONVIF_TIMEOUT,
    )
...
def element_to_data(node: ET.Element):
    children = list(node)
    result = {f"@{local_name(k)}": v for k, v in node.attrib.items()}
...
def get_profile_details() -> dict:
    ...
    body = envelope("<trt:GetProfiles/>", f'xmlns:trt="{TRT_NS}"')
    response = soap_post(MEDIA_URL, f"{TRT_NS}/GetProfiles", body)
...
def onvif_diagnostics() -> dict:
    ...
    cfg_response = soap_post(PTZ_URL, f"{TPTZ_NS}/GetConfiguration", cfg_body)
    cfg_root = ET.fromstring(cfg_response.content)
    ...
    node_response = soap_post(PTZ_URL, f"{TPTZ_NS}/GetNode", node_body)
    ...
    options_response = soap_post(PTZ_URL, f"{TPTZ_NS}/GetConfigurationOptions", options_body)
    ...
    status_body = envelope(
        f"<tptz:GetStatus><tptz:ProfileToken>{escape(profile['token'])}</tptz:ProfileToken></tptz:GetStatus>",
        f'xmlns:tptz="{TPTZ_NS}"',
    )
    status_response = soap_post(PTZ_URL, f"{TPTZ_NS}/GetStatus", status_body)
...
def onvif_continuous_move(command: str, pulse_seconds: float) -> dict:
    ...
    move_body = envelope(
        f"<tptz:ContinuousMove><tptz:ProfileToken>{escape(token)}</tptz:ProfileToken><tptz:Velocity>{velocity}</tptz:Velocity></tptz:ContinuousMove>",
    ...
    return {"start_status": started.status_code, "stop_status": stopped.status_code, "profile_token": token,
            "velocity": {"x": x, "y": y, "zoom": zoom}, "pulse_seconds": pulse_seconds}
...
def onvif_get_presets() -> list[dict]:
    ...
    root = ET.fromstring(soap_post(PTZ_URL, f"{TPTZ_NS}/GetPresets", body).content)
    presets = []
...
def onvif_set_preset(name: str, preset_token: str) -> dict:
    ...
    body = envelope(
        f"<tptz:SetPreset><tptz:ProfileToken>{escape(profile)}</tptz:ProfileToken>{name_xml}<tptz:PresetToken>{escape(preset_token)}</tptz:PresetToken></tptz:SetPreset>",
        f'xmlns:tptz="{TPTZ_NS}"',
    )
    response = soap_post(PTZ_URL, f"{TPTZ_NS}/SetPreset", body)
    root = ET.fromstring(response.content)
    token_node = root.find(f".//{{{TPTZ_NS}}}PresetToken")
    return {"status": response.status_code, "requested_token": preset_token,
            "preset_token": token_node.text if token_node is not None else None, "name": name}
...
<div class="preset-box"><strong>Saved positions</strong><span class="small">Refresh manually. Select an existing preset token, optionally edit its name, then save the current camera position back to that same preset.</span><select id="presetSelect"><option value="">Press Refresh list to load presets</option></select><input id="presetName" maxlength="40" placeholder="Preset name" disabled><button id="savePreset" class="save-preset" disabled>Save current position</button><div class="preset-actions"><button id="reloadPresets">Refresh list</button><button id="gotoPreset">Go to selected</button></div></div>
<div id="status" class="status">Ready</div><button id="restart" class="restart">Restart web service</button></div></div></div>
<div class="panel"><div class="tabs"><button class="tab-button active" data-tab="consoleTab">Console log</button><button class="tab-button" data-tab="diagnosticsTab">Camera diagnostics</button></div>
<div id="consoleTab" class="tab-panel active"><div class="footer-row"><h2>Console</h2><button id="clearConsole">Clear view</button></div><div id="console" class="console">Loading logs…</div></div>
<div id="diagnosticsTab" class="tab-panel"><div class="diag-toolbar"><div><h2>Camera diagnostics</h2><div id="diagTime" class="small">Not collected yet</div></div><button id="runDiagnostics" class="diag-button">Run read-only diagnostics</button></div><div id="diagnostics" class="diagnostics"><div class="diag-empty">Run diagnostics to query the camera's PTZ profile, configuration, node, supported spaces, ranges, and current status.</div></div></div></div>
</main><script>
const image=document.getElementById('view'),status=document.getElementById('status'),consoleBox=document.getElementById('console'),badge=document.getElementById('feedBadge'),feedToggle=document.getElementById('feedToggle'),stepInput=document.getElementById('ptzStep'),presetSelect=document.getElementById('presetSelect'),presetName=document.getElementById('presetName'),savePresetButton=document.getElementById('savePreset'),diagnosticsBox=document.getElementById('diagnostics'),diagTime=document.getElementById('diagTime'),runDiagnosticsButton=document.getElementById('runDiagnostics');let shownSequence=-1,clearedBefore=0,renderedLogCount=-1,feedEnabled=false;const savedStep=localStorage.getItem('ptzStep');if(savedStep)stepInput.value=savedStep;function normalizedStep(){{const value=Math.max(1,Math.min(10,parseInt(stepInput.value||'{DEFAULT_PTZ_STEP}',10)));stepInput.value=value;localStorage.setItem('ptzStep',value);return value}}stepInput.addEventListener('change',normalizedStep);stepInput.addEventListener('input',()=>{{if(stepInput.value!=='')localStorage.setItem('ptzStep',stepInput.value)}});
...
function renderDiagnostics(data){{diagnosticsBox.innerHTML='';for(const [section,value] of Object.entries(data)){{if(section==='collected_at')continue;const box=document.createElement('section');box.className='diag-section';const title=document.createElement('h3');title.textContent=section.replaceAll('_',' ');box.appendChild(title);const grid=document.createElement('div');grid.className='diag-grid';for(const [key,val] of flatten(value)){{const k=document.createElement('div'),v=document.createElement('div');k.className='diag-key';v.className='diag-value';k.textContent=key;v.textContent=val;grid.append(k,v)}}box.appendChild(grid);diagnosticsBox.appendChild(box)}}diagTime.textContent='Collected '+data.collected_at}}
async function runDiagnostics(){{runDiagnosticsButton.disabled=true;diagnosticsBox.innerHTML='<div class="diag-empty">Querying camera…</div>';status.textContent='Running read-only ONVIF diagnostics…';try{{const r=await fetch('/api/diagnostics',{{method:'POST'}}),j=await r.json();if(!r.ok)throw new Error(j.error||r.statusText);renderDiagnostics(j);status.textContent='Diagnostics completed'}}catch(e){{diagnosticsBox.innerHTML='';const d=document.createElement('div');d.className='diag-empty';d.textContent='Diagnostics failed: '+e;diagnosticsBox.appendChild(d);status.textContent='Diagnostics failed: '+e}}finally{{runDiagnosticsButton.disabled=false;loadLogs()}}}}
