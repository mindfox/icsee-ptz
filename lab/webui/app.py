...
function consoleSelectionActive(){const selection=window.getSelection();return !!(selection&&!selection.isCollapsed&&selection.anchorNode&&consoleBox.contains(selection.anchorNode))}
async function loadLogs(force=false){if(!force&&consoleSelectionActive())return;try{const r=await fetch('/api/logs',{cache:'no-store'});const j=await r.json();const entries=j.entries.slice(clearedBefore);if(!force&&entries.length===renderedLogCount)return;const wasNearBottom=consoleBox.scrollHeight-consoleBox.scrollTop-consoleBox.clientHeight<24;consoleBox.innerHTML='';for(const e of entries){const line=document.createElement('div');line.className=e.level.toLowerCase();line.textContent=`${e.timestamp} [${e.level}] ${e.message}`;consoleBox.appendChild(line)}renderedLogCount=entries.length;if(wasNearBottom||renderedLogCount<=2)consoleBox.scrollTop=consoleBox.scrollHeight}catch(e){consoleBox.textContent='Unable to load logs: '+e}}
